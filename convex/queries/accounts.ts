    ...
    handler: async (ctx, args) => {
      ...
      if (!isNaN(Number(args.searchTerm))) {
        ...
        const exactMatches = await ctx.db
          ...
          .filter((q) => 
            q.eq(q.field("school_id"), Number(args.searchTerm))
          )
          .collect();
      ...
      const filteredResults = results.filter(account => {
        ...
        return (
          (account.first_name && account.first_name.toLowerCase().includes(searchTermLower)) ||
          (account.middle_name && account.middle_name.toLowerCase().includes(searchTermLower)) ||
          (account.last_name && account.last_name.toLowerCase().includes(searchTermLower)) ||
